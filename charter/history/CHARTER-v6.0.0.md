...
- The change takes effect when the vote closes, and the Clerk records it as an
...
- **Safeguards:**
  ...
  - The Collective always keeps the Clerk and at least three voting members.
...
reproduce the Collective as of the current Charter version. The Chief of Staff tests
...
executing → post-mortem → human review → closed. The Chief runs the calendar
with `agents/bin/sprint.py`. Only one sprint is open at a time.

14.2 **One proposal each.** Every voting member, and the Chief, submits exactly
...
The Chief records each outcome on the edict (`edict.py note`), and whoever
...
16.4 **After every amendment,** the Clerk:
...
Class C) and computed by `agents/bin/metrics.py`. The Chief commits a daily
snapshot in `metrics/`.
...
site (for example on GitHub Pages) once the Chief accepts it and the Steward
...
- Media maintains it (task T-0002), and the Chief checks its accuracy.
- It's updated in the same sprint as any amendment or change to commands,
  controls, or the weekly rhythm, and reviewed weekly.
- It states which Charter version it matches. The Chief flags it in the
...
2. Media writes the post covering **every section** of the facts: sprints,
   each agent's activity, decisions, Charter changes, research, prototypes,
   media, governance, learnings, and metrics.
3. The Chief checks every claim against its sources.
...
- The Chief runs `agents/bin/seed-check.sh` at every daily digest. It
...
| Daily | Chief commits the metrics snapshot (Article 18) and runs the seed check (Article 20) |
| 08:00 | Chief digest: shipped, pending approvals, @rex questions, spend, incidents, drift check, event-log verify, commit both repos and push the private one, new cases, reviews due, precedent survival rate |
...
| When | Phase | Who |
|---|---|---|
| Sun evening | Sprint opens; each member writes one proposal | Chief, all members |
| Mon midday | Deliberation: positions, 2 debate rounds, one revision | fusion-harness session |
| Mon afternoon | Freeze, sealed ballots per item, tally | Clerk, voting members |
...
| Sun morning | Post-mortem: self-reports, sealed grades, report | Members, Clerk |
| Sun afternoon | Human review and follow-on actions; sprint closes; the next opens | Steward, Chief |
| Sun evening | Weekly facts compiled; blog drafted, source-checked, approved, published; User Guide reviewed | Media, Chief, Steward |
...
````markdown
<!-- Generated from CHARTER.md v6.0.0 — do not edit; amend the Charter -->
...
| **Social** (@VerafyAI) | Grok (configurable) | Drafting posts and replies from `private/outbox/approved` material and mentions | Rex (approval), then posting |
...
| **Media** | Claude Code | Recording demos, adding voiceover and captions, exporting video, and drafting posts | Social (via `private/outbox/pending`) |
...
11. **Sprints (Charter Article 14).** Check `python3 agents/bin/sprint.py
    ...
    - **deliberating / voting:** your governance voice argues and votes in the
      Clerk's fusion-harness session. You never vote on your own item.
...
- **Policy check** every file in `private/outbox/pending/` against POLICIES before Rex
...
- **Amendments (Charter Article 7):** you are the clerk.
  ...
  - When an amendment takes effect:
    ...
    3. append the hash-chained Amendment Log entry (use
       ...
       to `charter/history/CHARTER-vX.Y.Z.md`, run
       `agents/bin/charter-verify.py`, and run `agents/bin/charter.py
       materialize` to regenerate files;
...
- **Edicts (Charter Article 15):** read `private/edicts/INDEX.md` each run.
  For any edict with status `issued`:
  - implement it through the right channel: a Steward amendment draft for
    Rex to ratify, a steward case, or a board `#decision`;
  - record what you did with `python3 agents/bin/edict.py note E-NNNN
    "..."`.

  Never edit an edict's original words.
- **Sprint master (Charter Article 14).** Run the weekly calendar in
  `agents/config.env` with `agents/bin/sprint.py`:
  1. **Sunday:** `open` the next sprint, then write your own proposal.
  2. **Monday morning:** check proposals are complete (`check`) and nudge
     members who haven't submitted.
  3. **Monday midday:** deliberation session in fusion-harness (opening
     positions on every proposal, then 2 debate rounds). Owners may revise
     their proposal once afterwards.
  4. **Monday afternoon:** `freeze`, then the ballot session (sealed, one
     line per item). Export ballots to `sprints/<id>/ballots/<role>.md`,
     then `tally`.
  5. **Notify the Steward** with the tally and the plan preview. The Steward
     runs `sprint.py steward --approve [--veto ...]`, which writes `plan.md`
     and files a case per item.
  6. **Tuesday to Saturday:** hold members to the plan and flag drift.
  7. **Sunday:** `postmortem`, then a grading session (sealed grades), then
     `grade`, then send the report to the Steward for human review
     (`sprint.py review --file`).
  - Export every session's full transcript to `sprints/<id>/deliberation/`.
- **Reporter of case law (Charter Article 13):**
  ...
  - run `case.py check` daily. The digest lists new cases, cases due for
    ...
    (`case.py stats`).
- **Repositories (Charter Article 17):** at each daily digest, run
  `agents/bin/repos.sh commit "Daily state <date>"`.
  - Private material is committed to the private repo; the public repo is
    committed only if the redaction scan passes.
  - Then run `agents/bin/ledger-backup.sh`, which pushes the private repo.
  - Never push the public repo. The Steward does that, unless
    PUBLIC_AUTO_PUSH=1.
- **The seed (Charter P12, Article 20):** run `agents/bin/seed-check.sh` at
  every daily digest. A failure is an `#incident` and blocks that day's
  public commit until fixed.
- **Reopening (Article 13.11):** schedule a rehearing within one sprint for
  every `#reopen C-NNNN` thread that cites new information.
- **Membership votes (Article 3.6):** clerk them like amendments, as Class M
  in `gov-tally.py` with the subject set, and enforce the minimum of the
  Clerk plus three voters.
- **Credit (P4):** check that prototypes, videos, and posts credit their
  sources and that `CREDITS.md` is current. Nothing IP-restricted ships
  without an entry in `org/PERMISSIONS.md`.
- **Documentation (Charter Article 19):**
  - **Blog:** check every claim in Media's weekly blog draft against the facts
    file and its sources before it reaches the Steward. Annotate problems in
    `REVIEW.md` next to the draft.
  - **User Guide:** if `docs/USER-GUIDE.md` says "Matches Charter vX" and X
    isn't the current version, flag it in the digest and create a task for
    Media.
- **Observability (Charter Article 18):**
  - once `agents/bin/metrics.py` exists, run it daily and commit the snapshot
    in `metrics/`;
  - track task T-0001 (the dashboard) to acceptance against
    `specs/dashboard.md`;
  - keep `org/OKRS.md` marked DRAFT until the Steward confirms it by edict.
- **Structure over time (Charter Article 16):** after every amendment, run
  `agents/bin/charter.py timeline` and `agents/bin/charter.py tag`, and
  commit.
...
**Weekly blog (Charter Article 19):** every Sunday after the sprint's human
review.
1. Run `python3 agents/bin/weekly-digest.py`, which writes
   `blog/_facts/<date>.md`.
2. Write the week's post in plain, human-readable prose, 600–1,200 words,
   titled `# Week of <date>: <headline>`. It must **cover every section of
   the facts file**: sprint results, what each agent did, decisions, Charter
   changes, research, prototypes, media, governance counts, learnings, and
   metrics.
3. Every factual claim links its source (case, commit, file, or event
   number). Never include private material; counts only for edicts,
   incidents, and drafts.
4. Label it AI-written, and be honest about what didn't work.
5. Save it as `private/outbox/pending/<date>-blog-<slug>.md` and tag the Chief.

**User Guide (task T-0002):** you own `docs/USER-GUIDE.md`.
- Update it in the same sprint as any amendment or change to commands,
  controls, or the weekly rhythm, and bump its "Matches Charter vX" line.
- Review it every week when you write the blog.
...
CHIEF_TOOLS="Read,Write,Edit,Glob,Grep,Bash(tsk:*),Bash(date:*),Bash(python3 agents/bin/case.py:*),Bash(python3 agents/bin/sprint.py:*),Bash(python3 agents/bin/edict.py note:*),Bash(python3 agents/bin/edict.py replay:*)"
...
## V.22 `agents/bin/ledger-backup.sh`
...
MEMBERS = ["social", "researcher", "ideas", "prototyper", "media"]   # voting members
PROPOSERS = MEMBERS + ["chief"]                                        # Chief proposes, never votes
...
def main():
    ...
    if a.cmd == "new":
        ...
        secs = {"Original words": words,
                "Edict": a.restatement or "(Restatement to be written by the Chief as Reporter; the original words govern.)",
                "Outcome": "Pending. The Chief implements it through a Charter amendment and/or a steward case, and notes the result here.",
...
## V.27 `agents/bin/weekly-digest.py`
...
## V.29 `agents/bin/blog-publish.sh`
...
python3 agents/bin/eventlog.py record --actor media --type blog.published --data "{\"summary\": \"$(head -1 "$out" | sed 's/^# //; s/\"//g')\", \"file\": \"$out\"}"
echo "published $out (Steward pushes the public repo: agents/bin/repos.sh push --public)"
````

## V.30 `agents/bin/charter-hash.sh`
...
## V.31 `agents/bin/charter-verify.py`
...
## V.33 `agents/bin/gov-publish.sh`
...
## V.35 `governance/personas/_common.md`
...
## V.36 `governance/personas/clerk.md`
...
## V.37 `governance/personas/social.md`
...
## V.38 `governance/personas/researcher.md`
...
## V.39 `governance/personas/ideas.md`
...
## V.40 `governance/personas/prototyper.md`
...
## V.41 `governance/personas/media.md`
...
none
""")
for r in ["social", "researcher", "ideas", "prototyper", "media", "chief"]: proposal(r)
...
voters = ["social", "researcher", "ideas", "prototyper", "media"]
plan = {"S-0001-social": "yes", "S-0001-researcher": "yes", "S-0001-ideas": "no",
        "S-0001-prototyper": "yes", "S-0001-media": "yes", "S-0001-chief": "yes"}
for v in voters:
    lines = []
...
assert meta["S-0001-chief"]["outcome"] == "approved" and len(meta["S-0001-chief"]["votes"]) == 5
...
for g in voters:
    (sp / "postmortem/grades" / f"{g}.md").write_text("\n".join(
        f"{i} | 4 | 3 | 4 | 4 | 3 | note" for i in ["S-0001-social", "S-0001-researcher", "S-0001-prototyper", "S-0001-chief"]))
run("grade")
rep = (sp / "postmortem/report.md").read_text()
assert "| S-0001-social | social | 4 | 3 | 4 | 4 | 3 | 3.6 | A | 4 |" in rep, rep
...
## V.46 `tests/test_repos_charter.py`
...
## V.48 `research/library/LIBRARY.md`
...
## V.49 `research/library/fetch.sh`
...
## V.53 `herdr/projects/collective.toml`
...
[[tabs.panes]]
label = "Approvals (private/outbox/pending)"
...
## V.55 `charter/history/README.md`
...
## V.58 `org/board/2026-09-23-kickoff.md`
...
## V.59 `org/board/2026-09-24-amendment-case-law.md`
...
## V.60 `org/board/2026-09-24-amendment-collective.md`
...
## V.61 `org/board/2026-09-24-amendment-documentation.md`
...
## V.62 `org/board/2026-09-24-amendment-edicts.md`
...
## V.63 `org/board/2026-09-24-amendment-founding-documents.md`
...
## V.64 `org/board/2026-09-24-amendment-observability.md`
...
## V.65 `org/board/2026-09-24-amendment-public-governance.md`
...
## V.66 `org/board/2026-09-24-amendment-replayability.md`
...
## V.67 `org/board/2026-09-24-amendment-research-library.md`
...
## V.68 `org/board/2026-09-24-amendment-sprints.md`
...
## V.69 `org/board/2026-09-25-amendment-founding-principles.md`
...
## V.70 `org/board/2026-09-24-task-T-0001-dashboard.md`
...
## V.71 `org/board/2026-09-24-task-T-0002-user-guide-and-blog.md`
...
## V.72 `org/cases/C-0001-launch-sequence-and-draft-only-social.md`
...
## V.73 `org/cases/C-0002-public-governance-via-fusion-harness.md`
...
## V.74 `org/cases/C-0003-proposals-must-cite-the-research-library.md`
...
## V.75 `org/cases/C-0004-everything-is-a-replayable-event.md`
...
## V.76 `org/cases/C-0005-decisions-become-binding-case-law.md`
...
## V.77 `org/cases/C-0006-the-collective-dashboard-is-mandated.md`
...
## V.78 `org/cases/C-0007-the-collective-documents-itself.md`
...
## V.79 `org/cases/C-0008-the-twelve-founding-principles.md`
...
| [C-0006](C-0006-the-collective-dashboard-is-mandated.md) | The Collective Dashboard is mandated | steward | operations, tooling, transparency | 2026-09-24 | good_law | The Collective builds and maintains a local, read-only, fully versioned observability dashboard per specs/dashboard.md, as task T-0001. |
| [C-0007](C-0007-the-collective-documents-itself.md) | The Collective documents itself | steward | transparency, media, operations | 2026-09-24 | good_law | The Collective keeps a living User Guide in the public repo and publishes a weekly, human-readable, fully sourced blog post covering all of its activity, after Steward approval. |
...
- **C-0001** · good_law · cited by: C-0007 (follows)
...
- **C-0006** · good_law · cited by: C-0007 (follows)
- **C-0007** · good_law · cited by: C-0008 (follows)
- **C-0008** · good_law · cited by: not yet cited
````

## V.82 `research/papers.md`
...
committed by the Chief. Adding or changing a KPI is a Class C amendment.
...
| `sprints_closed` | Sprints with phase `closed` | sprints/ | 12 / quarter | Chief |
| `median_sprint_grade` | Median item grade across closed sprints | sprints/*/postmortem | ≥ B | Chief |
| `precedent_survival_rate` | From `case.py stats` | org/cases/ | ≥ 80% | Chief |
| `public_incidents` | Incidents about public output | private/incidents/, event log | 0 | Chief |
| `integrity_days` | Days on which all verifiers passed | event log (`ledger.backup`, verify results) | 100% | Chief |
| `edict_latency_days` | Days from edict issued to first outcome note | private/edicts/ | ≤ 7 | Chief |
| `approval_latency_hours` | Hours from a draft entering `private/outbox/pending` to approval or rejection | event log | ≤ 48 | Steward |
| `spend_by_role` | Model spend per role per day, from run transcripts | event log | within caps | Chief |
````

## V.85 `dashboard/README.md`
...
## V.86 `docs/USER-GUIDE.md`

````markdown
# The Collective: User Guide

> **Matches Charter v6.0.0.** Maintained by Media, with accuracy checked by
> the Chief (task T-0002, Charter Article 19). Updated in any sprint that
...
| Agent | Job |
|---|---|
| Chief of Staff | Runs the board, sprints, and governance (non-voting Clerk); files case law; checks drafts against policy; sends the daily digest |
| Social | The @VerafyAI voice (Grok); drafts posts and replies, and posts only what the Steward approved |
| Researcher | Finds and briefs papers on LLM-as-a-judge, agent scoring, and debate |
| Ideas | Turns briefs into small prototype proposals |
| Prototyper | Builds approved prototypes, and the dashboard |
| Media | Records demo videos; writes the weekly blog and this guide |
...
- Make a formal ruling: start a board thread with `#ruling`; the Chief files
...
| Symptom | Do this |
|---|---|
| An agent is looping or misbehaving | `touch org/PAUSE-<role>`, then read its run in the dashboard's live stream |
...
- `agents/bin/seed-check.sh` proves the Charter still recreates the whole
  Collective. The Chief runs it daily.
...
everything it did (Charter Article 19). Posts are written by Media from
`blog/_facts/<date>.md`, the week's facts compiled from versioned records.
They're checked against their sources by the Chief, approved by the Steward,
...
## V.94 `specs/dashboard.md`

````markdown
# The Collective Dashboard: spec (task T-0001)

**Mandate:** edict E-0031, steward case C-0006, Charter Article 18.
**Owner:** Prototyper (build). Media reviews the design; the Chief accepts it
against this spec. The setup session may build v1 during setup (setup plan,
Step 4).
...
- **`agents/bin/metrics.py`** computes every KPI from records, writes
  `metrics/YYYY-MM-DD.json`, and the Chief commits it daily. That gives the
...
6. Public mode, design review by Media, and acceptance by the Chief against
   this spec.
