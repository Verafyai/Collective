...
12.10 **The Weave mirror (edict E-0106).** The event log is also traced to the
Steward's private Weights & Biases Weave project (`agents/bin/weave_sync.py`,
run in the `weave` tab): each agent run as one trace holding its events, the
text of its prompt and transcript, and the cost its model reported; every
other event as its own call, at its recorded time. Everything is redacted by
the rules of 12.3 before it leaves the machine, and the mirror never carries
file contents, edict text, or `agents/.env`. The key is `WANDB_API_KEY` in
`agents/.env`. The mirror is a view, not a record: the event log stays the
only source of truth, and a Weave outage never stops recording.
...
[[tabs]]
name = "weave"
command = "agents/.venv/bin/python agents/bin/weave_sync.py follow --every 60"
````
...
## V.133 `agents/bin/weave_sync.py`
