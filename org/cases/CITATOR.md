# Citator

How later cases treat each case. Status is derived from these treatments.

- **C-0001** · limited · cited by: C-0007 (follows), C-0009 (limits), C-0011 (follows)
- **C-0002** · good_law · cited by: C-0005 (follows), C-0006 (follows), C-0008 (follows)
- **C-0003** · limited · cited by: C-0009 (limits), C-0011 (follows)
- **C-0004** · good_law · cited by: C-0005 (follows), C-0006 (follows), C-0011 (follows)
- **C-0005** · good_law · cited by: C-0008 (follows)
- **C-0006** · limited · cited by: C-0007 (follows), C-0009 (limits), C-0010 (follows), C-0011 (follows)
- **C-0007** · limited · cited by: C-0008 (follows), C-0009 (limits)
- **C-0008** · good_law · cited by: C-0009 (follows)
- **C-0009** · good_law · cited by: C-0010 (follows), C-0011 (follows)
- **C-0010** · good_law · cited by: C-0011 (follows)
- **C-0011** · good_law · cited by: not yet cited
- **C-0012** · good_law · cited by: not yet cited
